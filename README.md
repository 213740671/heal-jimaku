...
* **智能文本分割**: 深度整合 [DeepSeek API](https://platform.deepseek.com/)，利用其强大的语言理解能力及优化的处理提示词（支持日语、中文、英文），根据语义和标点符号进行自然断句。
...
* **配置持久化**:
    ...
    * 保存用户自定义的SRT生成参数和免费转录参数。
* **处理反馈**: 提供优化后的详细日志输出和进度条显示。
...
(注意：`packaging/file_version_info.txt` 文件用于 Windows 打包时定义版本信息，PyInstaller 会自动查找。当前版本为 `0.1.1.0`)
