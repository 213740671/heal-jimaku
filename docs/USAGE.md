...
2.  **DeepSeek API Key**: 您需要一个有效的 DeepSeek API Key。可以从 [DeepSeek 开放平台](https://platform.deepseek.com/) 注册并获取。此 API Key 主要用于文本的智能分割。**请注意：由于程序采用摘要预处理和分块技术来优化长文本的分割效果，（超长的）单个任务可能会产生多次API调用，请留意您的DeepSeek账户额度和使用情况。**
3.  **输入文件**:
...
1. 标题栏与窗口控制:
   ...
   - **设置按钮 (⚙️)**: 位于标题栏左侧，点击打开“自定义高级设置”对话框。
...
- **浏览... (JSON 文件)**: 点击打开文件对话框，选择包含语音文本和时间戳的 JSON 文件。此按钮在“免费获取JSON”模式下禁用。
   ...
   - **免费获取JSON 按钮**: 点击打开“JSON输出参数设置”对话框，允许上传音频文件并设置免费转录参数。
...
(注意：LLM调用温度目前由程序内部设定以保证分割效果，未来版本可能考虑在高级设置中提供调整选项。)
...
2. (可选) 调整高级SRT设置:
   ...
   - 点击主界面左上角的 **设置按钮 (⚙️)** 打开“自定义高级SRT参数”对话框。
   - 根据您的需求调整SRT参数，然后点击“确定”保存。如果不需要修改，可以跳过此步骤。
...
1. 输入 API Key:
   ...
   - 同上，确保 DeepSeek API Key 已设置。这是后续文本分割所必需的。

2. (可选) 调整高级SRT设置:

   - 同上，您可以预先设置好期望的SRT输出参数。
...
4. 设置转录参数并选择音频文件:
   ...
   - 点击“确定”。主界面将更新，JSON文件相关控件会被禁用，JSON输入框会显示您选择的音频文件名。
...
6. 开始转换:
   ...
   - 点击 "开始转换" 按钮。程序会首先调用免费STT服务进行在线转录，生成临时的JSON数据，然后使用此数据进行后续的DeepSeek智能分割和SRT优化。
...
1. 监控进度与日志:
   ...
   - 日志区域会输出详细信息，包括与API的交互（免费STT API 和 DeepSeek API）、**获取全文摘要的过程、文本如何被分块、逐块调用DeepSeek API进行分割的情况**、文本片段的对齐情况、字幕条目的调整等。优化后的日志逻辑会提供更清晰的步骤说明。如果出现任何问题，错误信息也会在此显示。
...
- 用户配置:
  ...
  - 内容: 保存 API Key (可选)、上次使用的JSON/音频路径、上次使用的输出路径、上次选择的JSON格式、上次输入模式，以及自定义的高级SRT参数和免费转录参数。
...
- "缺少信息" / "错误" 弹窗:
  ...
  - 确保 API Key (用于DeepSeek分割)、导出目录都已正确填写或选择。
...
- API 相关错误 (日志中显示):
  ...
  - **请求超时**: 网络连接问题或 DeepSeek API 服务繁忙。可以稍后重试。**由于长文本处理包含多次API调用（摘要+分块分割），请确保网络连接在整个处理过程中保持稳定。**
    - **API 响应格式错误/内容为空**: 可能是 DeepSeek API 临时问题，或输入文本过于特殊（例如过长，虽然已有分块但仍可能遇到边界情况）导致模型无法处理。**日志中会指明是摘要获取阶段还是特定文本块分割阶段出错。**
...
- "LLM 片段无法对齐" / "对齐相似度较低" (日志中显示):
  ...
  - 原因可能包括：
    - DeepSeek 对文本的改写程度较大（尽管优化的Prompt要求不增删字符，但模型行为有时难以完全控制）。新的分割逻辑引入了摘要作为上下文，理论上已经减少了模型对【当前文本块】随意改写的情况。
...
- 为啥一定要用DeepSeek的api，不能直接用免费接口或者其他更牛逼的大模型的接口嘛？
  - 最客观的原因是，在理解并分割日文文本（现已扩展至中文、英文）这个任务上，ds的性价比很高，兼顾了准确性和开销。
  ...
  - 具体测试效果如下面这张图所示。 可以发现，ds-v3在面对这种标点符号识别数量很少的文本时，也能在系统提示词（现已经已进一步优化，此次图片仅做效果展示）的指导下输出不逊于顶尖模型的结果。唯一的不同仅仅是语气词、感叹词或迟疑词的处理，而这些短促的句子，在后续字幕合并优化时间戳的逻辑中是可以被统一的。相较之下，另一个被我寄予厚望的模型——qwen3，由于思考时间过长影响效率，以及实际输出结果不佳的缘故，最终被pass掉了。
